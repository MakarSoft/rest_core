...
class MonobankAPIError(Exception):
    ...
    def __init__(
        self, http_response: HTTPResponse, custom_message: Optional[str] = None
    ):
    ...
    def __str__(self) -> str:
        ...
        try:
            raw_response: str = json.dumps(
                deserialized_response, indent=4, ensure_ascii=False
            )  # type: ignore
    ...
    def _deserialize_response(self) -> Dict[str, Any]:
        ...
        if self._deserialize_cache.get(id(self.http_response)) is not None:
            return cast(
                Dict[str, Any], self._deserialize_cache[id(self.http_response)]
            )
