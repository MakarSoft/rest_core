...
class Token:
    ...
    def __init__(
        ...
        *,
        ...
        before_expiration: Optional[int] = None # кол-во секунда, до истечения срока действия (от получения)
    ) -> None:
        
        self.token = token
        now = datetime.now()
        if not isinstance(initialization_time, datetime) or initialization_time > now:
    ...
    def is_valid(self) -> bool:
        ...
        if self.before_expiration:
            valid = self.initialization_time + timedelta(seconds=self.before_expiration) > datetime.now()
        return valid
    ...
    def get_saved_token(filename: str) -> Optional[Token]:
        ...
        if token_file_path.exists():
            with open(token_file_path) as token_file:
                token = json.load(
                    token_file,
                    object_hook=token_json_dict_decoder
                )
